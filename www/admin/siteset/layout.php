        ...
        <tr>
          <td align="center"><table width="95%" border="0" cellspacing="0" cellpadding="5">
		   ...
		   <?php for($i=0;$i<$intNews;$i++) {?>
            <tr>
              <td width="8%" height="20" align="left" valign="middle" class="txt_leftmenu">
			  <a href="layout.php?03<?php echo $arrnewsinfos[$i][id_layout];?>"  class="txt_leftmenu">
			  <?php  
			     $strSQL="SELECT opicname FROM layoutpic WHERE id_layout='".$arrnewsinfos[$i][id_layout]."' order by id_layoutpic desc limit 1";
                 $objDB->Execute($strSQL);
                 $arronepic=$objDB->fields();
				 echo '<img src="/upload/layout/'.$arronepic[opicname].'" width="50" height="50"  border="0" />';
			  ?>
              </a>              </td>
              <td width="72%" align="left" valign="middle" class="txt_leftmenu"><a href="layout.php?03<?php echo $arrnewsinfos[$i][id_layout];?>"  class="txt_leftmenu"><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></a></td>
              <td width="7%" align="left" valign="middle" class="txt_leftmenu"><?php if($arrnewsinfos[$i][openstat]=='1'){echo '打开';}else{echo '关闭';}?></td>
              <td width="13%" align="left" valign="middle" class="txt_leftmenu"><?php if($onuserperm_editprem=='1'){?>
                <a href="layout.php?03<?php echo $arrnewsinfos[$i][id_layout];?>"  class="txt_leftmenu">编辑</a>
                <?php }?>
                
                <? if($i!=0){ //判断一级目录是否位置在最前?>
<a href='layout.php?action=switch&id=<?=$arrnewsinfos[$i][id_layout];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i-1][id_layout];?>&swtorder=<?=$arrnewsinfos[$i-1][ordernum];?>'><img src='../inc/pics/icon_001.gif' border='0' width='10' height='10'></a>&nbsp;
			<? }else {?>
<img src='../inc/pics/icon_0012.gif' border='0' width='10' height='10'>&nbsp;
			 
			<? } if($i!=$intNews-1){//判断一级目录是否位置在最后?>
<a href='layout.php?action=switch&id=<?=$arrnewsinfos[$i][id_layout];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i+1][id_layout];?>&swtorder=<?=$arrnewsinfos[$i+1][ordernum];?>'><img src='../inc/pics/icon_002.gif' border='0' width='10' height='10'></a>&nbsp;
			<? } else{ ?>
          ...
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  <tr bgcolor="#FFFFFF">
                    <td width="91" valign="top" bgcolor="#FFFFFF">标题</td>
                    <td bgcolor="#FFFFFF"><input name="title" type="text" class="input_01" id="newstitle" style="width:500px" value="<?php echo $onenewsinfo[title];?>" <? if($onenewsinfo[id_layout]==1 || $onenewsinfo[id_layout]==2 || $onenewsinfo[id_layout]==3 || $onenewsinfo[id_layout]==4 || $onenewsinfo[id_layout]==5){echo 'readonly';}?> /></td>
                    </tr>
                                      <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                    </tr>
